...
mod tests {
    ...
    #[test]
    fn enum_decoders_accept_known_values_and_reject_unknown() {
        assert_eq!(HeightReference::from_i32(0), Some(HeightReference::Baro));
        assert_eq!(HeightReference::from_i32(99), None);

        assert_eq!(
            PositionReference::from_i32(2),
            Some(PositionReference::ExternalVision)
        );
        assert_eq!(PositionReference::from_i32(-1), None);

        assert_eq!(GnssMode::from_i32(1), Some(GnssMode::DeadReckoning));
        assert_eq!(GnssMode::from_i32(2), None);

        assert_eq!(GnssFixType::from_i32(5), Some(GnssFixType::RealTime));
        assert_eq!(GnssFixType::from_i32(6), None);

        assert_eq!(RangeControl::from_i32(2), Some(RangeControl::Enabled));
        assert_eq!(RangeControl::from_i32(7), None);

        assert_eq!(MagFusionType::from_i32(6), Some(MagFusionType::InitOnly));
        assert_eq!(MagFusionType::from_i32(-7), None);

        assert_eq!(BaroControl::from_i32(1), Some(BaroControl::Enabled));
        assert_eq!(BaroControl::from_i32(2), None);

        assert_eq!(
            OpticalFlowControl::from_i32(1),
            Some(OpticalFlowControl::Enabled)
        );
        assert_eq!(OpticalFlowControl::from_i32(2), None);

        assert_eq!(FlowGyroSource::from_i32(1), Some(FlowGyroSource::Internal));
        assert_eq!(FlowGyroSource::from_i32(-2), None);
    }

    #[test]
    fn bitflag_defaults_match_px4_defaults() {
        assert_eq!(GnssControl::DEFAULT, GnssControl::HPOS | GnssControl::VEL);
        assert_eq!(
            ImuControl::DEFAULT,
            ImuControl::GYRO_BIAS | ImuControl::ACCEL_BIAS
        );
    }

    #[test]
    fn from_bits_truncate_drops_unknown_bits() {
        let gnss_raw = GnssControl::HPOS.bits() | (1 << 29);
        let gnss = GnssControl::from_bits_truncate(gnss_raw);
        assert_eq!(gnss, GnssControl::HPOS);

        let imu_raw = ImuControl::ACCEL_BIAS.bits() | (1 << 30);
        let imu = ImuControl::from_bits_truncate(imu_raw);
        assert_eq!(imu, ImuControl::ACCEL_BIAS);
    }
}
